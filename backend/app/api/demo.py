...
def trigger_scenario_normal(db: Session = Depends(get_db)):
    """
    Scenario 1: Streams nominal, safe readings (pH: 7.2, Turbidity: 1.2 NTU, EC: 310 µS/cm, Temp: 27.0 °C).
    Expected: Anomaly is FALSE, Sensor Health is HEALTHY, Active Events cleared/resolved.
    """
...
def trigger_scenario_disturbance(db: Session = Depends(get_db)):
    """
    Scenario 2: Multi-sensor co-variation (Turbidity: 25.0 NTU, EC: 920 µS/cm, pH: 7.3, Temp: 27.5 °C).
    Expected: Anomaly is TRUE (score ~0.87), Event WATER_QUALITY_ANOMALY created, Sensor Health is HEALTHY.
    """
    payload = TelemetryIngestRequest(
        device_id="AQUA-01",
        timestamp=datetime.now(timezone.utc),
...
def trigger_scenario_sensor_fault(db: Session = Depends(get_db)):
    """
    Scenario 3: Single sensor extreme shift (pH: 2.0 while Turbidity, EC, Temp remain normal).
    Expected: Sensor Health is FAULT_SUSPECTED (suspect: pH), Event SENSOR_FAULT_SUSPECTED created.
    """
    payload = TelemetryIngestRequest(
        device_id="AQUA-01",
        timestamp=datetime.now(timezone.utc),
