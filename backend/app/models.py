...
class EventRecord(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    event_type = Column(String(64), nullable=False)   # e.g., "WATER_QUALITY_ANOMALY", "SENSOR_FAULT_SUSPECTED"
    severity = Column(String(32), nullable=False, default="MEDIUM")  # "LOW", "MEDIUM", "HIGH", "CRITICAL"
    device_id = Column(String(64), index=True, nullable=False, default="AQUA-01")
    timestamp = Column(DateTime, index=True, nullable=False, default=utc_now)
    anomaly_score = Column(Float, nullable=False, default=0.0)
    status = Column(String(32), nullable=False, default="ACTIVE")   # "ACTIVE", "RESOLVED", "ACKNOWLEDGED"
...
class SensorHealthRecord(Base):
    ...
    device_id = Column(String(64), index=True, nullable=False, default="AQUA-01")
    timestamp = Column(DateTime, index=True, nullable=False, default=utc_now)
    status = Column(String(32), nullable=False, default="HEALTHY")  # "HEALTHY", "FAULT_SUSPECTED", "DEGRADED"
    suspect_sensor = Column(String(64), nullable=True)             # e.g., "pH", "turbidity", "ec", "temperature"
    details = Column(Text, nullable=True)
