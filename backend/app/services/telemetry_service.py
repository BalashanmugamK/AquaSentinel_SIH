...
class TelemetryService:
    ...
    def ingest_reading(
        ...
        """
        Main pipeline: Ingest → Store → Analyze (Anomaly + Sensor Health) → Event Process
        """
        ...
        # 3. Run Sensor Health Evaluation (Fault Isolation)
        ...
        # 4. Event Processing with Dedup & Cooldown
    ...
    def get_latest_reading(cls, db: Session, device_id: str = "AQUA-01") -> Optional[TelemetryRecord]:
        """Fetch the most recent telemetry point."""
        return (
    ...
    def get_history(cls, db: Session, device_id: str = "AQUA-01", limit: int = 50) -> List[TelemetryRecord]:
        """Fetch historical readings ordered chronologically for charts."""
        records = (
            db.query(TelemetryRecord)
            .filter(TelemetryRecord.device_id == device_id)
            .order_by(TelemetryRecord.timestamp.desc())
        ...
        # Return in ascending order for smooth time-series rendering
        records.reverse()
        return records
